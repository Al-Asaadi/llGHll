...
local function lock_group_links(msg, data, target)
  ...
  if group_link_lock == 'مفعل' then
    return 'هذا الامر مفعل سابقاا 😸🖐🏿'
  else
    data[tostring(target)]['settings']['lock_link'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'تم قفل منع نشر الروابط 😸🖐🏿'
  end
end
...
local function unlock_group_links(msg, data, target)
  ...
  local group_link_lock = data[tostring(target)]['settings']['lock_link']
  if group_link_lock == 'غير مفعل' then
    return 'هذا الامر تم تفعيله سابقاا 😸🖐🏿'
  else
    data[tostring(target)]['settings']['lock_link'] = 'no'
    save_data(_config.moderation.data, data)
    return 'تم فتح منع نشر الروابط 😸🖐🏿'
  end
end
...
local function lock_group_spam(msg, data, target)
  ...
  if group_spam_lock == 'مفعل' then
    return 'هذا الامر تم تفعيله سابقاا 😸🖐🏿'
  else
    data[tostring(target)]['settings']['lock_spam'] = 'مفعل'
    save_data(_config.moderation.data, data)
    return 'تم قفل نشر كلايش التفليش الان مجموعتك محميه من التفليش 😸🖐🏿'
  end
end
...
local function unlock_group_spam(msg, data, target)
  ...
  local group_spam_lock = data[tostring(target)]['settings']['lock_spam']
  if group_spam_lock == 'غير مفعل' then
    return 'تم فتح  التفليش بالفعل 😸🖐🏿'
  else
    data[tostring(target)]['settings']['lock_spam'] = 'no'
    save_data(_config.moderation.data, data)
    return 'تم فتح نشر كلايش التفليش قم با قفلها للحماية 😸🖐🏿'
  end
end
...
local function lock_group_flood(msg, data, target)
  ...
  if group_flood_lock == 'مفعل' then
    return 'تم قفل التكرار سابقاا 😸🖐🏿'
  else
    data[tostring(target)]['settings']['flood'] = 'مفعل'
    save_data(_config.moderation.data, data)
...
local function unlock_group_flood(msg, data, target)
  ...
  local group_flood_lock = data[tostring(target)]['settings']['flood']
  if group_flood_lock == 'غير مفعل' then
...
local function lock_group_arabic(msg, data, target)
  ...
  if group_arabic_lock == 'yes' then
    return 'اللغة العربية بالفعل مقفوله 😸🖐🏿!'
  else
...
local function unlock_group_arabic(msg, data, target)
  ...
  if group_arabic_lock == 'no' then
    return 'اللغة العربيه بالفعل مفتوحه 😸🖐🏿!'
  else
    data[tostring(target)]['settings']['lock_arabic'] = 'no'
    save_data(_config.moderation.data, data)
    return 'تم فتح اللغه العربية 😸🖐🏿!'
  end
end
...
local function lock_group_membermod(msg, data, target)
  ...
  if group_member_lock == 'مفعل' then
    return 'الاضافة مقفولة سابقاا 😸🖐🏿'
  else
...
local function unlock_group_membermod(msg, data, target)
  ...
  local group_member_lock = data[tostring(target)]['settings']['lock_member']
  if group_member_lock == 'غير مفعل' then
    return 'الاضافه مفتوحة بالفعل 😸🖐🏿'
  else
...
local function lock_group_rtl(msg, data, target)
  ...
  if group_rtl_lock == 'مفعل' then
    return 'الاضافة الجماعية بالفعل مقفولة😸🖐🏿!'
  else
    data[tostring(target)]['settings']['lock_rtl'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'تم قفل الاضافة الجماعية 😸🖐🏿!'
  end
end
...
local function unlock_group_rtl(msg, data, target)
  ...
  local group_rtl_lock = data[tostring(target)]['settings']['lock_rtl']
  if group_rtl_lock == 'غير مفعل' then
    return 'الاضافة الجماعية بالفعل مفتوحة 😸🖐🏿!'
  else
    data[tostring(target)]['settings']['lock_rtl'] = 'no'
    save_data(_config.moderation.data, data)
    return 'تم فتح الاضافة الجماعية 😸🖐🏿!'
  end
end
...
local function lock_group_tgservice(msg, data, target)
  ...
  if group_tgservice_lock == 'مفعل' then
    return 'اشعارات الدخول مقفولة سابقاا 😸🖐🏿!'
  else
    data[tostring(target)]['settings']['lock_tgservice'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'تم قفل اشعارات الدخول 😸🖐🏿!'
  end
end
...
local function unlock_group_tgservice(msg, data, target)
  ...
  local group_tgservice_lock = data[tostring(target)]['settings']['lock_tgservice']
  if group_tgservice_lock == 'غير مفعل' then
    return 'اشعارات الدخول مفتوحة سابقاا 😸🖐🏿!'
  else
    data[tostring(target)]['settings']['lock_tgservice'] = 'no'
    save_data(_config.moderation.data, data)
    return 'تم فتح اشعارات الدخول 😸🖐🏿'
  end
end
...
local function lock_group_sticker(msg, data, target)
  ...
  if group_sticker_lock == 'مفعل' then
    return 'الملصقات مقفولة بالفعل 😸🖐🏿'
  else
...
local function unlock_group_sticker(msg, data, target)
  ...
  local group_sticker_lock = data[tostring(target)]['settings']['lock_sticker']
  if group_sticker_lock == 'غير مفعل' then
...
local function lock_group_contacts(msg, data, target)
  ...
  if group_contacts_lock == 'مفعل' then
    return 'جهات الاتصال بالفعل مقفولة 😸🖐🏿!'
  else
...
local function unlock_group_contacts(msg, data, target)
  ...
  local group_contacts_lock = data[tostring(target)]['settings']['lock_contacts']
  if group_contacts_lock == 'غير مفعل' then
    return 'جهات الاتصال بالفعل مفتوحة 😸🖐🏿!'
  else
...
local function set_public_membermod(msg, data, target)
  ...
  if group_public_lock == 'مفعل' then
...
local function unset_public_membermod(msg, data, target)
  ...
  if group_public_lock == 'غير مفعل' then
...
local function promote_admin(receiver, member_username, user_id)
  ...
  if data[group]['moderators'][tostring(user_id)] then
    return send_large_msg(receiver, member_username..'انت بالفعل ضمن الاداره 😸🖐🏿!.')
  end
...
local function demote_admin(receiver, member_username, user_id)
  ...
  if not data[group]['moderators'][tostring(user_id)] then
    return send_large_msg(receiver, member_tag_username..'انت بالفعل ضمن الاعضاء 😸🖐🏿!.')
  end
...
local function promote2(receiver, member_username, user_id)
  ...
  local member_tag_username = string.gsub(member_username, '@', '(at)')
  if not data[group] then
    return send_large_msg(receiver, 'المجموعة ليست فعاله 😸🖐🏿!.')
  end
  if data[group]['moderators'][tostring(user_id)] then
    return send_large_msg(receiver, member_username..' أنت فعلاً أدمن 😸🖐🏿.')
  end
  data[group]['moderators'][tostring(user_id)] = member_tag_username
  save_data(_config.moderation.data, data)
  send_large_msg(receiver, member_username..' تم رفعك أدمن 😸🖐🏿.')
end
...
local function demote2(receiver, member_username, user_id)
  ...
  if not data[group]['moderators'][tostring(user_id)] then
    return send_large_msg(receiver, member_tag_username..' انت ليس أدمن 😸🖐🏿.')
  end
  data[group]['moderators'][tostring(user_id)] = nil
  save_data(_config.moderation.data, data)
  send_large_msg(receiver, member_username..' تم شحتك من الادمنيه 😸🖐🏿.')
end
...
function get_message_callback(extra, success, result)
    ...
    if is_momod2(member_id, channel_id) and not is_admin2(msg.from.id) then
			   return send_large_msg("channel#id"..channel_id, "لا يمكنك طرد ادمن او ادراري 😸🖐🏿!")
    end
    if is_admin2(member_id) then
         return send_large_msg("channel#id"..channel_id, "لا يمكنك طرد ادراري 😸🖐🏿!")
    end
	...
	elseif get_cmd == "setowner" then
		...
		local channel_id = 'channel#id'..result.to.peer_id
			...
			if result.from.username then
				...
				text = "[ "..result.from.peer_id.." ] تم الرفع مشرف 😸🖐🏿"
			end
			send_large_msg(channel_id, text)
		end
...
local function in_channel_cb(cb_extra, success, result)
  ...
  if member then
    text = 'لا يوجد عضو @'..member..' في هذه المجموعة 😸🖐🏿.'
  else
    text = 'لا يوجد عضو ['..memberid..'] في هذه المجموعة 😸🖐🏿.'
  end
	...
	for k,v in pairs(result) do
		...
		if vusername == member or vpeer_id == memberid then
			...
			if group_owner then
				...
				if result.username then
					text = member_username.." ["..v.peer_id.."] تم رفعك مدير للمجموعه 😸🖐🏿!"
				else
					text = "["..v.peer_id.."] تم رفعك مدير للمجموعه 😸🖐🏿!"
				end
			end
		elseif memberid and vusername ~= member and vpeer_id ~= memberid then
			...
			if group_owner then
				...
				text = "["..memberid.."] تم رفعك مدير للمجموعه 😸🖐🏿!"
			end
		end
	end
 end
	...
	local data = load_data(_config.moderation.data)
		if matches[1] == 'تفعيل' and not matches[2] then
			...
			if is_super_group(msg) then
				return reply_msg(msg.id, 'المجموعه مفعله سابقاا 😸🖐🏿.', ok_cb, false)
			end
		...
		if matches[1] == 'الغاء تفعيل' and is_admin1(msg) and not matches[2] then
			if not is_super_group(msg) then
				return reply_msg(msg.id, 'تم الغاء تفعيل المجموعة 😸🖐🏿.', ok_cb, false)
			end
		...
		if matches[1] == 'طرد' and is_momod(msg) then
			if type(msg.reply_id) ~= "nil" then
				local cbreply_extra = {
				...
				get_message(msg.reply_id, get_message_callback, cbreply_extra)
			elseif matches[1] == 'طرد' and matches[2] and string.match(matches[2], '^%d+$') then
			...
			elseif matches[1] == "طرد" and matches[2] and not string.match(matches[2], '^%d+$') then
			--[[local cbres_extra = {
					channelid = msg.to.id,
					get_cmd = 'channel_block',
					sender = msg.from.id
		...
		if matches[1] == 'ايدي' then
			...
			elseif msg.text:match("@[%a%d]") then
				...
				return "🔶 ايدي مجموعة " ..string.gsub(msg.to.print_name, "_", " ").. ":\n\n"..msg.to.id
			end
		end
		...
		if matches[1] == 'رابط جديد' and is_momod(msg)then
			...
			local receiver = get_receiver(msg)
				if success == 0 then
					send_large_msg(receiver, '* خطأ: فشل لاسترداد الرابط السبب : غير معروف..\n\n اذا كان لديك رابط, يرجى استخدام /وضع رابط المقرر أن تعيينها')
					data[tostring(msg.to.id)]['settings']['set_link'] = nil
					save_data(_config.moderation.data, data)
				else
					send_large_msg(receiver, "انشاء رابط جديد 🌝🖇")
		...
		if matches[1] == 'وضع رابط' and is_owner(msg) then
			...
			save_data(_config.moderation.data, data)
			return 'الرجاء ارسال رابط مجموعه الجديد 😐🖐🏿'
		end
		...
		if matches[1] == 'الرابط' then
			...
			if not group_link then
				return "الانشاء الرابط استخدم /رابط جديد اولاً!\n\nاو قم بوضع رابط ب ستخدام /وضع رابط انت تضع الرابط"
			end
		...
		if matches[1] == 'رفع ادمن' then
		  if not is_momod(msg) then
			...
			if not is_owner(msg) then
				return "فقط المشرف/المطور يستطيع رفع ادمن 😧💕"
			end
			if type(msg.reply_id) ~= "nil" then
				local cbreply_extra = {
					get_cmd = 'promote',
		...
		if matches[1] == 'تنزيل ادمن' then
			...
			if not is_owner(msg) then
				return "فقط مشرف او المطور يستطيع تنزيل ادمن 😧💕"
			end
			if type(msg.reply_id) ~= "nil" then
				local cbreply_extra = {
					get_cmd = 'demote',
			...
			elseif matches[1] == 'demote' and matches[2] and string.match(matches[2], '^%d+$') then
				...
				user_info(user_id, cb_user_info, {receiver = receiver, get_cmd = get_cmd})
			elseif matches[1] == 'تنزيل ادمن' and matches[2] and not string.match(matches[2], '^%d+$') then
				local cbres_extra = {
					channel = get_receiver(msg),
					get_cmd = 'demote'
				}
		...
		if matches[1] == "ضع وصف" and is_momod(msg) then
			...
			return "تم حفظ الوصف 😸✋🏿 .\n\nأذا تريد مشاهدة الوصف اضغط على اعداداات الكروب 😊✋🏿"
		end
		...
		if matches[1] == "ضع معرف" and is_admin1(msg) then
			local function ok_username_cb (extra, success, result)
				...
				elseif success == 0 then
					send_large_msg(receiver, "فشل تعيين معرف للمجموعه 😸🖐🏿!.\nUsername may already be taken.\n\nNote: Username can use a-z, 0-9 and underscores.\nMinimum length is 5 characters.")
				end
			end
			local username = string.gsub(matches[2], '@', '')
		...
		if matches[1] == 'ضع صورة' and is_momod(msg) then
			...
			return 'قم الان بارسال صورة ليتم حفظها 😸🖐🏿'
		end
		...
		if matches[1] == 'حذف' then
			...
			if not is_momod(msg) then
				return "فقط المشرف يستطيع الحذف 😸🖐🏿"
			end
			if matches[2] == 'الادمنيه' then
				if next(data[tostring(msg.to.id)]['moderators']) == nil then
					return 'لا يوجد ادمنيه ليتم حذفهم 😸🖐🏿'
				end
				for k,v in pairs(data[tostring(msg.to.id)]['moderators']) do
			...
			if matches[2] == 'المعرف' and is_admin1(msg) then
				local function ok_username_cb (extra, success, result)
					...
					elseif success == 0 then
						send_large_msg(receiver, "عذراً فشل في مسح المعرف 😸🖐🏿!.")
					end
				end
				local username = ""
				channel_set_username(receiver, username, ok_username_cb, {receiver=receiver})
			end
			if matches[2] == "البوت" and is_momod(msg) then
		...
		if matches[1] == 'ضع التكرار' then
			...
			if tonumber(matches[2]) < 2 or tonumber(matches[2]) > 35 then
				return "ضع التكرار من 2 الئ 35 😸🖐🏿!"
			end
			local flood_max = matches[2]
			data[tostring(msg.to.id)]['settings']['flood_msg_max'] = flood_max
			save_data(_config.moderation.data, data)
			savelog(msg.to.id, name_log.." ["..msg.from.id.."] set flood to ["..matches[2].."]")
			return 'تم تعين التكرار للعدد 😸🖐🏿: '..matches[2]
		end
		 if matches[1] == 'المراقبة' and is_momod(msg) then
			local target = msg.to.id
			if matches[2] == 'مفعل' then
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] set group to: public")
				return set_public_membermod(msg, data, target)
			end
			if matches[2] == 'غير مفعل' then
		...
		if matches[1] == 'قفل' and is_owner(msg) then
			...
			local msg_type = 'الكتابة'
				if not is_muted(chat_id, msg_type..': yes') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: mute "..msg_type)
					mute(chat_id, msg_type)
					return msg_type.." تم قفلها 😸🖐🏿!"
				else
					return "مقفله  "..msg_type.." بالفعل 😸🖐🏿!"
				end
			end
			if matches[2] == 'المحادثة' then
			local msg_type = 'المحادثة'
				if not is_muted(chat_id, msg_type..': yes') then
					savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: mute "..msg_type)
					mute(chat_id, msg_type)
					return "تم قفل "..msg_type.." 😸🖐🏿"
				else
					return "تم قفل "..msg_type.." بالفعل  😸🖐🏿" 
				end
			end
		...
		if matches[1] == 'فتح' and is_momod(msg) then
			...
			local msg_type = 'الصوتيات'
				if is_muted(chat_id, msg_type..': yes') then
					...
					return msg_type.." تم فتحها 😸🖐🏿"
				else
					return "تم فتح "..msg_type.." بالفعل  😸🖐🏿"
				end
			end
			if matches[2] == 'الصور' then
			local msg_type = 'الصور'
		...
		if matches[1] == "كتم" and is_momod(msg) then
			...
			elseif matches[1] == "كتم" and matches[2] and string.match(matches[2], '^%d+$') then
				local user_id = matches[2]
				if is_muted_user(chat_id, user_id) then
					unmute_user(chat_id, user_id)
			...
			elseif matches[1] == "كتم" and matches[2] and not string.match(matches[2], '^%d+$') then
				local receiver = get_receiver(msg)
				local get_cmd = "mute_user"
				local username = matches[2]
				local username = string.gsub(matches[2], '@', '')
