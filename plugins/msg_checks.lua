...
if is_chat_msg(msg) or is_super_group(msg) then
	...
	if settings.strict then
		...
		if msg and not msg.service and is_muted(msg.to.id, 'المحادثة: yes') or is_muted_user(msg.to.id, msg.from.id) and not msg.service then
		...
		if msg.media then -- msg.media checks
			...
			if is_muted(msg.to.id, 'الصور: yes') and msg.media.type:match("photo") or is_photo_caption and not msg.service then
				delete_msg(msg.id, ok_cb, false)
				if strict == "yes" or to_chat then
					--	kick_user(msg.from.id, msg.to.id)
				end
			...
			if is_muted(msg.to.id, 'المتحركة: yes') and is_gif_caption and msg.media.type:match("document") and not msg.service then
				delete_msg(msg.id, ok_cb, false)
				if strict == "yes" or to_chat then
					--	kick_user(msg.from.id, msg.to.id)
				end
			end
			if is_muted(msg.to.id, 'الصوتيات: yes') and msg.media.type:match("audio") and not msg.service then
				delete_msg(msg.id, ok_cb, false)
				if strict == "yes" or to_chat then
					kick_user(msg.from.id, msg.to.id)
				end
			...
			if  is_muted(msg.to.id, 'الفيديوهات: yes') and msg.media.type:match("video") and not msg.service then
				delete_msg(msg.id, ok_cb, false)
				if strict == "yes" or to_chat then
					kick_user(msg.from.id, msg.to.id)
				end
